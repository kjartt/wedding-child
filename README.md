...
## Локальный выбор оформления

В верхней части экрана доступны три варианта: «Приглашение», «Олива» и «Вечер».
Выбор сохраняется только в браузере (`localStorage`) и отражается в параметре `?style=`:
`paper`, `olive` или `evening`. Это позволяет согласовать оформление на локальном frontend;
само переключение темы ничего не публикует. В опубликованную версию изменения попадут только
после отдельного запуска `npm run deploy` из backend-репозитория.
